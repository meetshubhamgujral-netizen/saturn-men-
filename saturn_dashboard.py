...
# ==========================================
# FAIL-SAFE ENVIRONMENT CHECK FOR PLOTLY
# ==========================================
try:
...
# Global error banner if packages failed to sync on Streamlit Cloud
if not plotly_available:
    st.error("🚨 **System Environment Sync Delay detected by Saturn AI Engine**")
    st.warning(
        "Streamlit Cloud is still building your environment container in the background. "
        "Please look at the bottom-right corner of this webpage, click **'Manage app'**, "
        "click the **three vertical dots (⋮)** at the top of the log console, and select **'Reboot app'** "
        "to force-trigger the installation of `requirements.txt` instantly."
    )
...
# ==========================================
    ...
    if not plotly_available:
        st.error("This section requires `plotly`. Please perform an app reboot inside the 'Manage app' panel to finalize container setup.")
...
# ==========================================
    ...
    if not plotly_available:
        st.error("This section requires `plotly` to render analytical visuals. Please complete an app container reboot.")
    elif st.session_state.cleaned_df is None:
                ...
                with st.spinner("Executing model engineering convergence routine..."):
                    ...
                    if is_numeric:
                        ...
                        if plotly_available:
                            ...
                            st.info("Feature importance plotting bypassed because visualization layers are initializing.")
                    else:
